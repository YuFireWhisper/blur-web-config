...
import { Alert, Button, CircularProgress, Card, CardHeader, CardBody } from '@heroui/react';

export const LocationPage: React.FC = () => {
  const { serverIndex } = useParams();
  const { value: locations, isLoading, error } = useConfigValue<ConfigBlock[]>(
    `childrenBlocks.http.0.childrenBlocks.server.${serverIndex}.childrenBlocks.location`
  );
  const { updateConfig } = useConfigApi();
  
  // State for each location block
  ...
  useEffect(() => {
    if (locations) {
      const newStates = locations.reduce((acc, location, index) => {
        const initial = location.configItems?.reduce((values, item) => {
  ...
  const handleParamChange = (locationId: string, path: string, newValue: string) => {
  ...
  return (
    <div className="w-auto flex flex-col p-[5vw] gap-6">
      ...
      {locations.map((location, index) => {
        ...
        return (
          <Card key={locationId} className="w-full p-[2vw]">
            ...
            <CardBody>
              <ConfigItemTable
                ...
                onChange={(path, value) => handleParamChange(locationId, path, value)}
              />
            </CardBody>
          </Card>
        );
